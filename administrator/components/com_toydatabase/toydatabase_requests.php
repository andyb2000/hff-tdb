...
/*			$userid_mail="Hi,
		...
		switch($frm_in_status) {
			...
			case "1":
						...
						->where($db->quoteName('function') . ' = '. $db->quoteName("email_booktoy_approve"));
						$db->setQuery((string) $query_email1);
						$db->execute();
						$query_email1_rows= $db->getNumRows();
				if ($db->getNumRows() > 0) {
		...
		$userid_mail.="$loan_status
			...
			default:
				...
				$query_email1->select('*')
						->from($db->quoteName('#__toydatabase_permissions'))
						->where($db->quoteName('function') . ' = '. $db->quoteName("email_booktoy_reject"));
						$db->setQuery((string) $query_email1);
						$db->execute();
						$query_email1_rows= $db->getNumRows();
				if ($db->getNumRows() > 0) {
